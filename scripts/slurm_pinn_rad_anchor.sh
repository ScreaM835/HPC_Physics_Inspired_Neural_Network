...
CONFIG=configs/zerilli_l2_rad_anchor.yaml
CKPT_DIR="$WORKDIR/outputs/pinn/zerilli_l2_rad_anchor/checkpoints"

requeue_handler() {
    echo "[SIGNAL] Caught USR1 — job approaching time limit."
    echo "[SIGNAL] Resubmitting job to continue training..."
    sbatch "$WORKDIR/scripts/slurm_pinn_rad_anchor.sh"
    exit 0
}
...
echo ""
echo "============================================"
echo "[PINN] Training FNN + RAD with anchor retention..."
...
echo "  Anchor retention: 20% of old points kept"
