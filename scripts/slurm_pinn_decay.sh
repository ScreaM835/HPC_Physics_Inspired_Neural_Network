...
# RAD adaptive sampling + decay factoring (g = exp(t/tau)*Psi).
# ============================================================
#SBATCH --job-name=qnm_pinn_decay
...
CONFIG=configs/zerilli_l2_rad_decay.yaml
CKPT_DIR="$WORKDIR/outputs/pinn/zerilli_l2_rad_decay/checkpoints"
...
requeue_handler() {
    ...
    sbatch "$WORKDIR/scripts/slurm_pinn_decay.sh"
    exit 0
}
...
echo ""
echo "============================================"
echo "[PINN] Training FNN + RAD + Decay Factoring..."
...
echo "  Decay factoring: g = exp(t/tau)*Psi, tau=11.241"
echo "  g-equation: g_tt - g_xx - (2/tau)*g_t + (V + 1/tau^2)*g = 0"
