...
# RAD adaptive sampling only (no exponential reweighting).
# Reproduces the previous successful RAD result (RMSD=0.00124).
# ============================================================
...
CONFIG=configs/zerilli_l2_rad.yaml
CKPT_DIR="$WORKDIR/outputs/pinn/zerilli_l2_rad/checkpoints"

requeue_handler() {
    echo "[SIGNAL] Caught USR1 — job approaching time limit."
    echo "[SIGNAL] Resubmitting job to continue training..."
    sbatch "$WORKDIR/scripts/slurm_pinn_rad_only.sh"
    exit 0
}
...
echo ""
echo "============================================"
echo "[PINN] Training FNN + RAD only (no exp weight)..."
...
echo "  RAD: period=1000, k=1.0, c=1.0, candidates=100000"
