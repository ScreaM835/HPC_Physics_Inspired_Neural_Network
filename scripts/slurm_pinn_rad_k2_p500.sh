...
# RAD tuned: k=2.0, c=0.1, period=500 (sharper + more frequent)
# ============================================================
#SBATCH --job-name=qnm_pinn_k2p5
...
CONFIG=configs/zerilli_l2_rad_k2_p500.yaml
CKPT_DIR="$WORKDIR/outputs/pinn/zerilli_l2_rad_k2_p500/checkpoints"

requeue_handler() {
    echo "[SIGNAL] Caught USR1 — job approaching time limit."
    echo "[SIGNAL] Resubmitting job to continue training..."
    sbatch "$WORKDIR/scripts/slurm_pinn_rad_k2_p500.sh"
    exit 0
}
...
echo ""
echo "============================================"
echo "[PINN] Training FNN + RAD tuned (k=2, c=0.1, period=500)..."
...
echo "  RAD: period=500, k=2.0, c=0.1, candidates=100000"
